...
const NAV = [["overview", "Dashboard"], ["farmer", "Farmer onboarding"], ["agent", "Revenue officer desk"], ["registry", "Land registration"], ["transfer", "Mutation & transfer"], ["documents", "RTC & documents"], ["accounts", "Officer accounts"], ["analytics", "Gas analysis"], ["audit", "Audit register"]];
const PORTALS = {
  farmer: { label: "Farmer portal", account: "farmer", defaultView: "farmer", views: ["overview", "farmer", "transfer", "documents"] },
  ...
  purchaser: { label: "Purchaser portal", account: "buyer", defaultView: "transfer", views: ["overview", "transfer", "documents"] },
        ...
        <div className="rtc-certificate-paper">
          ...
          <div className="rtc-body-grid">
            <table className="rtc-details-table">
              <tbody>
                ...
                <tr><th>Khatedar / Owner Wallet</th><td>{land.owner}</td></tr>
          ...
          <div className="rtc-footer">
            <div>
              ...
              <p>Date: <strong>{new Date().toLocaleDateString('en-IN', { dateStyle: 'full' })}</strong></p>
            </div>
            <div className="rtc-seal">
              <p>OFFICIAL DIGITAL RTC</p>
              <p>Karnataka Land Registration</p>
            </div>
          </div>
        </div>
      </div>
...
function App() {
  ...
  const [message, setMessage] = useState("Loading the local demonstration authority...");
  ...
  const [documentForm, setDocumentForm] = useState({ landId: "9002", category: "RTC / Pahani extract", reference: "", hash: "" });
  ...
  const portal = session ? PORTALS[session.user.role] : null;
  const visibleNav = portal ? NAV.filter(([id]) => portal.views.includes(id)) : [];
  const accountRole = portal?.label || (!wallet ? "Connecting" : isRegistrar ? "Revenue registrar" : isCurrentOwner ? "Recorded owner" : isPendingOwner ? "Purchaser" : "Citizen / purchaser");
  const myLandRequests = farmer ? landRequests.filter((item) => item.farmerId === farmer.id) : [];
...
const DEMO_KEYS = {
  ...
  async function useDemoAccount(role) {
    try {
      const localProvider = new ethers.JsonRpcProvider(RPC_URL);
      const network = await localProvider.getNetwork();
      if (Number(network.chainId) !== 31337) throw new Error("Demo accounts are available only on the local Hardhat network.");
      const account = DEMO_ACCOUNTS[role];
      const privateKey = DEMO_KEYS[role];
      const signer = privateKey ? new ethers.Wallet(privateKey, localProvider) : await localProvider.getSigner(account);
      setWallet({ provider: localProvider, signer, account, chainId: Number(network.chainId) });
      setForm((current) => ({ ...current, owner: role === "farmer" ? account : current.owner || DEMO_ACCOUNTS.farmer, buyer: DEMO_ACCOUNTS.buyer }));
      setMessage(`${role === "authority" ? "Revenue Officer" : role === "farmer" ? "Farmer" : "Purchaser"} portal connected to the local blockchain.`);
  ...
  async function signIn(authResult) {
    ...
    if (!PORTALS[role]) throw new Error("This account does not have a portal role.");
    await useDemoAccount(PORTALS[role].account);
    setSession({ ...authResult, signedInAt: new Date().toISOString() });
    setView(PORTALS[role].defaultView);
    window.location.hash = `/${role}`;
    if (role === "farmer") {
      setFarmer({ id: authResult.user.id, name: authResult.user.fullName, email: authResult.user.email, mobile: authResult.user.mobile, walletAddress: DEMO_ACCOUNTS.farmer, verified: true });
      try {
        const savedFarmers = await api(`/api/farmers?email=${encodeURIComponent(authResult.user.email)}`);
        if (savedFarmers[0]) setFarmer(savedFarmers[0]);
  ...
  useEffect(() => {
    let active = true;
    const refresh = async () => { try { const localProvider = new ethers.JsonRpcProvider(RPC_URL); const [block, feeData] = await Promise.all([localProvider.getBlockNumber(), localProvider.getFeeData()]); if (active) setChainStats({ block, gasPrice: feeData.gasPrice || 0n }); } catch { if (active) setChainStats(null); } };
  ...
  useEffect(() => {
    ...
    new ethers.Contract(address, COMMON_ABI, wallet.provider).registrars(wallet.account).then((value) => { if (active) setIsRegistrar(value); }).catch(() => { if (active) setIsRegistrar(null); });
    return () => { active = false; };
  }, [wallet, address]);
  useEffect(() => {
    if (view === "transfer") {
  ...
  async function signerFor() { return wallet?.signer; }
  async function findLand(landId = form.lookupId) {
    try {
      if (landId) localStorage.setItem("bhoomi_active_land_id", String(landId));
      const result = await contract().getLandDetails.staticCall(landId);
      const item = variant === "base" ? { id: result[0].toString(), survey: result[1], location: result[2], area: result[3].toString(), owner: result[4], pendingOwner: result[5], status: Number(result[6]), history: result[7] } : { id: String(landId), metadataHash: result[2], area: result[1].toString(), owner: result[0], pendingOwner: result[3], status: Number(result[4]), history: result[5] };
      setLand(item); setForm((current) => ({ ...current, landId: String(landId), lookupId: String(landId) })); setMessage("Land record retrieved from the blockchain.");
      return item;
    } catch (error) { setMessage(displayError(error)); setLand(null); return null; }
  }
  async function submit(action) {
    let registry;
    try {
      setBusyAction(action); const signer = await signerFor(action); registry = contract(true, signer);
      if (["register", "approve"].includes(action) && !(await registry.registrars(await signer.getAddress()))) throw new Error("Registration and approval require Demo authority.");
      let tx;
      if (action === "register") {
      ...
      if (action === "request") { if (!ethers.isAddress(form.buyer)) throw new Error("Buyer wallet address is invalid."); tx = await registry.requestTransfer(form.landId, form.buyer); }
  ...
  async function submitLandRequest(event) {
    ...
    try {
      const request = await api("/api/land-requests", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ farmerId: farmer.id, surveyNumber: form.survey, district: form.district, taluk: form.taluk, hobli: form.hobli, village: form.village, extent: form.area, walletAddress: form.owner || wallet?.account || "" }) });
  ...
  return <div className="app-shell">
    ...
    {certificateLand && <RtcCertificateModal land={certificateLand} contractAddress={address} onClose={() => setCertificateLand(null)} />}
  </div>;
}
