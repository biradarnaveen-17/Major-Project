...
function App() {
  ...
  const allMyHoldings = useMemo(() => {
    const list = [...myLandRequests];
    if (land && (land.owner?.toLowerCase() === wallet?.account?.toLowerCase() || (session?.user?.fullName && resolveName(land.owner).toLowerCase().includes(session.user.fullName.toLowerCase())))) {
    ...
    return list;
  }, [myLandRequests, land, wallet, session]);
